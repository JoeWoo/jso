...
Result_term = Struct.new(:value,:x_id,:y_id)
...
class Tyccl
  ...
  items=[]
  @IDsIndex = Hash.new
  f = File.new(File.dirname( __FILE__) + "/dex/tyccl/cilin.txt")
  i=0
  f.each { |line|
  ...
  #++
  File.open(File.dirname(__FILE__)+"/dex/tyccl/Inverted.yaml") do  |f|
  ...
  # Returns a Struct Result_term which contains idA, idB, and shortest
  # semantic distance(int) between wordA and wordB.
  def self.dist(wordA, wordB)
    alpha=10.0/5
    shortest_Pair = Result_term.new(100,"","")
  ...
  # Given wordA(string) and wordB(string).
  # Returns a Struct Result_term which contains the most similar Pairs
  ...
  def self.sim(wordA, wordB)
    ...
    longest_Pair = Result_term.new(-1,"","")
    idAs = get_ids_by_word(wordA)
    idBs = get_ids_by_word(wordB)
