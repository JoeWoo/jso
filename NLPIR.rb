...
Result_t = struct ['int start','int length',"char  sPOS[#{POS_SIZE}]",'int iPOS','int word_ID','int word_type','double weight']
...
def NLPIR_FileProcess(sSourceFilename, sResultFilename, bPOStagged=1)
	NLPIR_FileProcess_rb.call(sSourceFilename, sResultFilename, bPOStagged)
	# fs=File.new(sSourceFilename,"r")
	# fr=File.new(sResultFilename,"w+")
	# fr.write NLPIR_ParagraphProcess(fs.read,bPOStagged)
	# fs.close
	# fr.close
end
